// community/pages/order/shopCart.js
...
Page({
  ...
  xuan_func: function() {
    ...
    var allselect = false;
    var all_cant_buy = 1;
    let { is_open_vipcard_buy, is_vip_card_member } = this.data;
   ...
   */
  calcAmount: function() {
    let { is_open_vipcard_buy, is_vip_card_member, carts, delivery_tuanz_money, man_free_tuanzshipping, vipcard_save_money, canLevelBuy, level_save_money, allcount, is_open_fullreduction } = this.data;
    ...
    cartsArr.forEach(key => {
      ...
      shopcarts.forEach(function(item) {
        ...
        if (item.isselect && man_free_tuanzshipping > 0 && delivery_tuanz_money > 0) {
          ...
          } else if (canLevelBuy && item.is_mb_level_buy == 1){
        ...
        //等级优惠
        if (canLevelBuy == 1 && item.is_mb_level_buy == 1 && item.isselect) {
    ...
    if (is_open_vipcard_buy == 1 && is_vip_card_member == 1) {
      totalAmount = (totalAmount - vipcard_save_money*1).toFixed(2);
    } else if (canLevelBuy) {
      totalAmount = (totalAmount - level_save_money * 1).toFixed(2);
    }
    totalAmount = totalAmount <= 0 ? 0 : totalAmount;

    let canbuy_tot = allcount * 1; //合计 计算满多少下单使用
    if(is_open_fullreduction==1) { totalAmount = allcount * 1; }
