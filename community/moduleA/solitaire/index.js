...
Page({
    ...
    shareCommunityId && util.getCommunityById(shareCommunityId).then(res => {
      ...
      if (res.open_danhead_model == 1) {
        ...
        if(shareCommunityId!=default_head_info.communityId) {
          let { groupInfo } = that.data;
          app.util.message(`您只能访问自己${groupInfo.group_name}`, 'switchTo:/community/pages/index/index', 'error', '知道了');
          return;
        }
      } else {
        // 社区是否存在
    ...
    app.util.request({
      ...
      success: function(res) {
        ...
        if (res.data.code == 0) {
          ...
          // 团长不存在
          app.util.message(res.data.msg, '/community/pages/position/community', 'error');
          return;
        } else {
  ...
  goDetails: function (event){
    ...
    let link = `/community/moduleA/solitaire/details?id=${id}`;
  ...
  onShareAppMessage: function () {
    ...
    let share_path = `community/moduleA/solitaire/index?share_id=${share_id}&community_id=${head_id}`;
    let title = '';
    return {
      title,
      path: share_path,
