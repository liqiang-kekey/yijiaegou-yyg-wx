...
Page({
  ...
  myinfo:function(){
    wx.navigateTo({
      url: '/community/pages/myinfo/index',
    })
  },
  /**
   * 生命周期函数--监听页面加载
...
          }          
  ...
  goToGroup: function() {
    5 === this.data.auditStatus ? wx.navigateTo({
      url: "/community/moduleA/groupCenter/index"
    }) : wx.navigateTo({
      url: "/community/moduleA/groupCenter/apply"
    });
  },
  ...
  goDistribution: function() {
    ...
    if (member_info.comsiss_flag == 0) {
      ...
      if (member_info.comsiss_state == 0) {
        ...
        //分销商已审核
        wx.navigateTo({
          url: '/community/distributionCenter/pages/me',
        })
      }
    }
  },
  ...
  distributionNext: function() {
    if (this.data.commiss_sharemember_need == 1) {
      console.log('需要分享');
      let url = '/community/distributionCenter/pages/recruit';
      wx.navigateTo({
        url
      })
    } else if (this.data.commiss_biaodan_need == 1) {
      console.log('需要表单');
      // let url = '/community/pages/distribution/apply';
      wx.navigateTo({
        url: '/community/distributionCenter/pages/recruit',
      })
    } else {
      // 跳转表单自动审核
      let status = 0;
  ...
  goNext: function(e) {
    ...
    if (type == 'share') {
      wx.navigateTo({
        url: '/community/distributionCenter/pages/share',
      })
