...
Page({
  ...
  onLoad: function (options) {
    ...
    let id = options.id || 0;
    if(!id) {
      app.util.message('参数错误', 'redirect:/community/moduleA/solitaire/groupIndex', 'error');
    }
  },
  ...
  onShow: function () {
    let id = this.options && this.options.id || 0;
    id && this.getData(id), this.getCommentList(), this.getOrderList();
  },
    ...
    app.util.request({
      ...
      success: function (res) {
        if (res.data.code == 0) {
          ...
          app.util.message('您还未登录', 'switchTo:/community/pages/index/index', 'error');
          return;
        } else {
          app.util.message(res.data.msg, 'redirect:/community/moduleA/solitaire/groupIndex', 'error');
          return;
        }
      }
    })
