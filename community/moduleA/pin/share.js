...
Page({
    ...
    app.util.request({
      ...
      success: function (res) {
        ...
        if (res.data.code == 0) {
          ...
          app.util.message('无数据', '/community/moduleA/pin/index', 'error');
        }
      }
    })
  },
        ...
        if (currentCommunityId != '' && shareCommunityId) {
          ...
          if (currentCommunityId != shareCommunityId) {
            ...
            if (hideCommunityChangeBtn == 1 && is_all_sale!=1) {
              app.util.message(`您只能访问自己${groupInfo.group_name}`, '/community/moduleA/pin/index', 'error', '知道了');
              return;
            }
            that.setData({
        ...
        if (res.confirm) {
          ...
          app.util.request({
            ...
            success: function (res) {
              if (res.data.code == 0) {
                wx.redirectTo({
                  url: `/community/moduleA/pin/goodsDetail?id=${goods_id}`
                })
              } else {
                wx.redirectTo({
                  url: '/community/moduleA/pin/index'
                })
              }
            }
          })
  ...
  goOrder: function () {
    ...
    util.addCart(data).then(res=>{
      if(res.showVipModal==1) {
        ...
        var pages_all = getCurrentPages();
        let url = `/community/pages/order/placeOrder?type=${buy_type}`;
  ...
  onShareAppMessage: function () {
    ...
    var share_path = 'community/moduleA/pin/share?id=' + that.data.order_id + '&share_id=' + share_id;
