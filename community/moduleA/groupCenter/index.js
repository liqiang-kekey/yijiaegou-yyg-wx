...
Page({
    ...
    app.util.request({
      ...
      success: function (res) {
        if (res.data.code == 0) {
          ...
          let { head_today_pay_money, today_add_head_member, today_after_sale_order_count, today_invite_head_member,is_open_solitaire } = res.data;
          that.setData({
  ...
  goOrder: function (e) {
    ...
    wx.navigateTo({
      url: "/community/moduleA/groupCenter/groupList?tab=" + status
    });
  },
  ...
  goEdit: function () {
    wx.navigateTo({
      url: "/community/moduleA/groupCenter/setting?id=" + this.data.community_info.id
    });
  },
  ...
  onShareAppMessage: function(res) {
    ...
    if(!id) return;
    return {
      title: shopname + ' ' +community_name || '',
      path: "community/pages/index/index?community_id=" + id,
      imageUrl: '',
      success: function() {},
      fail: function() {}
    };
