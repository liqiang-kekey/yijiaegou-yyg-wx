...
Page({
  ...
  onLoad: function (options) {
    ...
    if (!id) {
      app.util.message('参数错误', 'redirect:/community/moduleA/solitaire/index', 'error');
    }
    this.soli_id = id;
  },
  ...
  initFn(){
    ...
    this.setData({
      ...
      id && that.getData(id), that.getCommentList();
    })
  },
  ...
  onShow: function () {
    let that = this;
    let id = this.options && this.options.id || 0;
    id && this.getData(id), this.getCommentList(),this.getOrderList(), this.getMyOrderList();
    ...
    app.util.request({
      ...
      success: function (res) {
        if (res.data.code == 0) {
          ...
          app.util.message(res.data.msg, 'redirect:/community/moduleA/solitaire/index', 'error');
          return;
        }
      }
    })
    ...
    shareCommunityId && util.getCommunityById(shareCommunityId).then(res=>{
      ...
      if (res.open_danhead_model == 1) {
        ...
        if(shareCommunityId!=default_head_info.communityId) {
          let { groupInfo } = that.data;
          console.log('开启单社区');
          app.util.message(`您只能访问自己${groupInfo.group_name}`, 'redirect:/community/moduleA/solitaire/index', 'error', '知道了');
          return;
        }
      } else {
        // 社区是否存在
  ...
  goPlaceOrder: function(){
    ...
    this.showCartGoods(false).then(()=>{
      let soli_id = soli_info.id || '';
      let url = `/community/pages/order/placeOrder?type=soitaire&soli_id=${soli_id}`;
      wx.navigateTo({ url })
    }).catch(()=>{
      wx.showToast({
        title: '请先选择商品！',
  ...
  drawImg: function (head, soli_info) {
    ...
    let content = soli_info.content.replace(/<\/?.+?>/g, "");
    let option = [];
    let h = 300
  ...
  onShareAppMessage: function () {
    ...
    let share_path = `community/moduleA/solitaire/details?id=${soli_info.id}&share_id=${share_id}`;
    return {
      title,
      path: share_path,
      success: function (res) {
