...
Page({
  ...
  authSuccess: function () {
    ...
    let url = '/community/moduleA/pin/goodsDetail?id=' + id + '&scene=' + scene;
    ...
    util.addCart(data).then(res=>{
      if(res.showVipModal==1) {
        ...
        if (pages_all.length > 3) {
          wx.redirectTo({
            url: `/community/pages/order/placeOrder?type=${buy_type}&is_limit=${is_limit}`
          })
        } else {
          wx.navigateTo({
            url: `/community/pages/order/placeOrder?type=${buy_type}&is_limit=${is_limit}`
  ...
  onShareAppMessage: function () {
    ...
    var share_path = `community/moduleA/pin/goodsDetail?id=${goods_id}&share_id=${share_id}&community_id=${community_id}`;
