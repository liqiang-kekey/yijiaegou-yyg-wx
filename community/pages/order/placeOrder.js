...
Page({
    ...
    app.util.request({
      ...
      success: function (res) {
        console.log(res);console.log('=========')
         wx.hideLoading();
         that.setData({
           'new_region':res.data.new_region,
           'new_region_msg':res.data.new_region_msg,
           'is_drainage':res.data.is_drainage,
           'is_drainage_msg':res.data.is_drainage_msg,
         })
         if(res.data.is_drainage == "in_drainage"){
          wx.showModal({
            title: '友情提示',
            content: res.data.is_drainage_msg,
            showCancel: false,
          });
        }
        if(res.data.new_region == 'notin_region'){
          wx.showModal({
            title: '友情提示',
            content: res.data.new_region_msg,
            showCancel: false,
          });
        }
  ...
  prepay: function(){
    this.canPreSub = true;
    let that = this;
    if(this.data.ck_yupay == 1){
      wx.showModal({
        title: '提示',
        content: '您确认使用余额支付购买的商品？',
        showCancel: true,
        confirmColor: '#13bb4e',
        success:function(res){
          console.log(res);
          if (res.cancel) {
            console.log('用户取消余额支付');
            return;
          }else{
            console.log('用户确认余额支付');
            that.goodsPay();
          }
        }
      });     
  ...
  goodsPay: function() {  
    let { tabAddress, tabIdx, is_limit_distance_buy } = this.data;
    if (is_limit_distance_buy == 1 && (tabIdx == 1)){
      ...
      if(this.data.is_drainage == "in_drainage"){
        wx.showModal({
          title: '友情提示',
          content: this.data.is_drainage_msg,
          showCancel: false,
        });
        return;
      }
    if(this.data.new_region == 'notin_region'){
      wx.showModal({
        title: '友情提示',
        content: that.data.new_region_msg,
        showCancel: false,
        success(res) {
        }
      });
            ...
            if (has_yupay == 1) {
              ...
              if (buy_type == "dan" || buy_type == "pindan" || buy_type == "integral" || buy_type == "soitaire") {
                if (res.data.is_go_orderlist <= 1) {
                  wx.redirectTo({
                    url: '/community/pages/order/order?id=' + order_id + '&is_show=1'
                  })
                } else {
                  wx.redirectTo({
                    url: '/community/pages/order/index?is_show=1'
                  })
                }
              } else {
                wx.redirectTo({
                  url: `/community/moduleA/pin/share?id=${order_id}`
                })
              }
            } else {
              wx.requestPayment({
                ...
                'fail': function (error) {
                  if (res.data.is_go_orderlist <= 1) {
                    wx.redirectTo({
                      url: '/community/pages/order/order?id=' + order_id + '&?isfail=1'
                    })
                  } else {
                    wx.redirectTo({
                      url: '/community/pages/order/index?isfail=1'
                    })
                  }
                }
              })
  ...
  bindRegionChange: function (e) {
    ...
    region && this.checkOut(region[1],region[2]);
    this.setData({ region })
  },

  checkOut: function (mb_city_name,mb_counties_name) {
    var that = this;
    var token = wx.getStorageSync('token');
    var community = wx.getStorageSync('community');
    var community_id = community.communityId;
    ...
    app.util.request({
      ...
      success: function (res) {
       ...
       if(res.data.new_region == 'notin_region'){
        ...
        if(res.data.code==1){
          let rdata = res.data;
          let { vipcard_save_money, shop_buy_distance, is_limit_distance_buy, current_distance, level_save_money, score, score_for_money, bue_use_score } = rdata;
  ...
  choseLocation: function() {
    ...
    wx.chooseLocation({
      success: function (e) {
        ...
        function setRes(){
          console.log('setData');
          s_region && (s_region[1] != "市") && that.checkOut(s_region[1],s_region[2]);
        ...
        if (res.authSetting['scope.address']) {
          wx.chooseAddress({
            success(res) {
              ...
              that.setData({ tabAddress })
              region && (region[1] != "市") && that.checkOut(region[1],region[2]);
            },
            fail(res){
              console.log("step4")
              console.log(res)
  ...
  calcPrice: function(isTabSwitch = 0){
    let tdata = this.data;
    let { total_free, delivery_tuanz_money, fare_man_shipping_fare_money, trans_free_toal, tabIdx, goods, is_open_vipcard_buy, is_member_level_buy, is_vip_card_member, canLevelBuy, fare_man_delivery_tuanz_fare_money } = tdata;
